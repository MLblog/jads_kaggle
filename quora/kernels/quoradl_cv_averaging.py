...
# train, validation split
# x_train, x_val, y_train, y_val = train_test_split(train_X, train["target"], shuffle=True, test_size=0.1)
# load embeddings and add non-existant lower case words to the embedding dictionary
embedding_dict = load_word2vec(EMBEDDING_FILE, return_as_dict=True)
embedding_dict = add_lower(embedding_dict, training, testing)
embedding_matrix = create_embedding_matrix(tokenizer.word_index, embedding_dict)

## Run the bunch and create submission
predictions = cross_validate_and_predict(train_X.values, train_Y, test_X.values, folds=5)
