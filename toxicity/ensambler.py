...
class Ensambler(object):
    def __init__(self, train_y, test_id, tags, data_dir='data/output/'):
        """
        This class creates an ensamble solution from the predictors. The models
        can be ensambled using its mean or XGBoots
    ...
    @staticmethod
    def get_model_val(models_name, data_dir, val_source='test'):
        """
    ...
    def _xgboost_ensambler(self, params):
        """
        Ensambler method that uses XGBoost

        Parameters
        -------------------------------------------------------------------
        params: the parameters to tune the XGBoost algorithm
        """
        models_name = self.get_models_name(self.data_dir)
        if type(models_name) != list:
            models_name = [models_name]

        get_train_y = self.get_model_val(models_name, self.data_dir, 'train')
        get_test_y = self.get_model_val(models_name, self.data_dir, 'test')
        train = pd.DataFrame()
        test = pd.DataFrame()
        ...
        best_params, best_score = bayesian_optimization(XGBPredictor, train, self.train_y, params, model_type='GP', acquisition_type='EI',
        ...
        xgb_predictor = XGBPredictor(**best_params)
        create_submission(xgb_predictor, train, self.train_y, test, self.train_id, self.test_id,
                          write_to='data/output/submission_xgboost_ensambler.csv', data_source_nature='sparce_matrix', to_ensamble=False)

    def _mean_ensambler(self):
        """
        Ensambler method that uses the average
        ...
        doc_name = self.data_dir + '/' + 'submission_average_ensambler.csv'
