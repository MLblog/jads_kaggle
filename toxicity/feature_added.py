...
class FeatureAdded(object):
    ...
    def add_features(self, train, test):
        if self.upper_case: self._upper(train), self._upper(test)
        if self.word_count: self._count_words(train), self._count_words(test)
        if self.unique_words_count: self._unique_words(train), self._unique_words(test)
        if self.letter_count: self._count_letters(train), self._count_letters(test)
        if self.punctuation_count: self._count_punctuation(train), self._count_punctuation(test)
        if self.little_case: self._count_little_case(train), self._count_little_case(test)
        if self.stopwords: self._count_stopwords(train), self._count_stopwords(test)
        if self.question_or_exclamation: self._question_or_exclamation(train), self._question_or_exclamation(test)
        return  train, test
